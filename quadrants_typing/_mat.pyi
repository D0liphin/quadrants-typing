...
class Mat(DType, Generic[_Num, _Row, _Col]):
    ...
    @overload
    def __init__(self: Mat[_NumAlt, _RowAlt, _ColAlt], fill: _NumInit, /) -> None: ...
    @overload
    def __init__(
    ...
    # `*` is elementwise (`Self`) or a scale (`_Num`); the matrix product is `@`.
    def __mul__(self, other: Self | _Num, /) -> Self: ...
    def __rmul__(self, other: _Num, /) -> Self: ...
    def __truediv__(self, other: _Num, /) -> Self: ...
    # `(R x K) @ (K x C) -> (R x C)`; the shared inner dimension `K_` must match.
    @overload
    def __matmul__[R_: DimAny, K_: DimAny, C_: DimAny](
        self: Mat[_Num, R_, K_], other: Mat[_Num, K_, C_], /
    ) -> Mat[_Num, R_, C_]: ...
    # `(R x C) @ (C,) -> (R,)`; column count must match the vector length.
    @overload
    def __matmul__[R_: DimAny, C_: DimAny](
        self: Mat[_Num, R_, C_], other: Vec[_Num, C_], /
    ) -> Vec[_Num, R_]: ...
