...
# `pyfunc` is `func` that is additionally callable from plain Python, so it
# shares `func`'s signature.
@overload
def pyfunc[**P, R](fn: Callable[P, R], /) -> qdt.Func[P, R]: ...
@overload
def pyfunc(*, is_real_function: bool = ..., requires_top_level: bool = ...) -> _FuncDecorator: ...
...
# Typed as the class so it stays usable as an annotation and a checked constructor; the
# runtime object is a `lang.struct.StructType`, reach its `members` through a cast.
@dataclass_transform()
...
@overload
def select(
    cond: qdt.Vec[_Num2, _Dim], x1: qdt.Vec[_Num, _Dim] | _Num, x2: qdt.Vec[_Num, _Dim] | _Num
) -> qdt.Vec[_Num, _Dim]: ...
@overload
def select(
    cond: qdt.Mat[_Num2, _Row, _Col],
    x1: qdt.Mat[_Num, _Row, _Col] | _Num,
    x2: qdt.Mat[_Num, _Row, _Col] | _Num,
) -> qdt.Mat[_Num, _Row, _Col]: ...
@overload
